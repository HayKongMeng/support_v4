...
<div class="flex flex-col lg:flex-row gap-6">
    ...
    <div class="flex-1">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                <h2 class="text-lg font-semibold text-gray-800"><?= __('v_knowledge_base') ?></h2>
                <p class="text-sm text-gray-500"><?= __('v_find_answers_and_share_knowledge') ?></p>
            </div>
            ...
            <form method="GET" action="<?= $app->url('kb') ?>" class="flex flex-col md:flex-row gap-4">
                <div class="flex-1">
                    <div class="relative">
                        <input type="text" name="search" value="<?= htmlspecialchars($search ?? '') ?>"
                            placeholder="<?= __('v_search_articles') ?>"
                ...
                <div class="w-full md:w-48">
                    <select name="category" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        <option value=""><?= __('v_all_categories') ?></option>
        ...
        <!-- Featured Articles -->
        <div class="mb-8">
            <h3 class="text-md font-semibold text-gray-900 mb-4"><i class="fas fa-star text-yellow-500 mr-2"></i><?= __('v_featured_articles') ?></h3>
        ...
        <div class="bg-white rounded-lg shadow p-8 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-2"><?= __('v_no_articles_found') ?></h3>
            <p class="text-gray-600 mb-4"><?= __('v_be_the_first_to_share_knowledge') ?></p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="bg-white rounded-lg shadow hover:shadow-md transition-shadow">
                <div class="p-5">
                    ...
                    $copyText = trim($article['title'] . "\n\n" . $copyContent);
                    ?>
                    <div class="mt-3 flex items-center justify-end">
                        <button type="button" onclick="copyKbText('kb-copy-<?= $article['id'] ?>')"
                                class="text-xs text-gray-500 hover:text-indigo-600">
                            <i class="fas fa-copy mr-1"></i> <?= __('v_copy_full_text') ?>
                        </button>
                    </div>
                    <textarea id="kb-copy-<?= $article['id'] ?>" class="sr-only" aria-hidden="true"><?= htmlspecialchars($copyText) ?></textarea>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
        ...
        <div class="flex justify-center mt-6">
            <nav class="flex items-center space-x-2">
                ...
                <span class="px-4 py-2 text-gray-700">
                    <?= __('v_page') ?> <?= $pagination['current_page'] ?> <?= __('v_of') ?> <?= $pagination['total_pages'] ?>
                </span>
    ...
    <div class="w-full lg:w-64 space-y-4">
        ...
        <div class="bg-white rounded-lg shadow p-4">
            <h3 class="font-semibold text-gray-900 mb-3"><i class="fas fa-pen mr-2"></i><?= __('v_my_articles') ?></h3>
            <a href="<?= $app->url('kb/my-articles') ?>" class="text-indigo-600 hover:text-indigo-800 text-sm">
                <?= __('v_view_my_articles') ?> <i class="fas fa-arrow-right ml-1"></i>
            </a>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-4">
            <h3 class="font-semibold text-gray-900 mb-3"><i class="fas fa-fire text-orange-500 mr-2"></i><?= __('v_popular') ?></h3>
            <div class="space-y-3">
        ...
        <!-- Categories -->
        <div class="bg-white rounded-lg shadow p-4">
            <h3 class="font-semibold text-gray-900 mb-3"><i class="fas fa-tags mr-2"></i><?= __('v_categories') ?></h3>
            <div class="space-y-2">
                <?php foreach ($categories as $cat): ?>
