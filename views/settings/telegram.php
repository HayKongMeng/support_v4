        ...
        <form action="<?= $app->url('settings/telegram') ?>" method="POST" class="p-6 space-y-6">
            ...
            <div class="p-4 bg-gray-50 rounded-lg space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="font-medium text-gray-900">Assigned Ticket Alerts</h3>
                        <p class="text-sm text-gray-500">Notify assigned agents when a ticket is assigned or a customer replies</p>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="notify_assigned_agents" value="1"
                               <?= ($telegramConfig['notify_assigned_agents'] ?? false) ? 'checked' : '' ?>
                               class="sr-only peer">
                        <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-indigo-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                    </label>
                </div>
                <div>
                    <label for="alert_group_chat_id" class="block text-sm font-medium text-gray-700 mb-2">Alert Group Chat ID (optional)</label>
                    <input type="text" id="alert_group_chat_id" name="alert_group_chat_id"
                           value="<?= htmlspecialchars($telegramConfig['alert_group_chat_id'] ?? '') ?>"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                           placeholder="-1001234567890">
                    <p class="text-xs text-gray-500 mt-1">If set, alerts are also sent to this group chat</p>
                </div>
            </div>
