...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm">
        <div class="p-6 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-500 mt-1">Fill in the details below to create a support ticket</p>
        </div>
        ...
        <form action="<?= $app->url('tickets') ?>" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="requester_name" class="block text-sm font-medium text-gray-700 mb-2">Customer Name *</label>
                ...
                <div>
                    <label for="requester_email" class="block text-sm font-medium text-gray-700 mb-2">Customer Email *</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Leave empty to let AI categorize</p>
                </div>
                <div>
                    ...
                    <select id="priority" name="priority"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                        <option value="">Auto-detect (AI)</option>
                        <option value="low" <?= ($oldInput['priority'] ?? '') === 'low' ? 'selected' : '' ?>>Low</option>
                        <option value="medium" <?= ($oldInput['priority'] ?? '') === 'medium' ? 'selected' : '' ?>>Medium</option>
                        <option value="high" <?= ($oldInput['priority'] ?? '') === 'high' ? 'selected' : '' ?>>High</option>
                        <option value="urgent" <?= ($oldInput['priority'] ?? '') === 'urgent' ? 'selected' : '' ?>>Urgent</option>
                    </select>
                </div>
                <div>
            ...
            <div class="p-4 bg-indigo-50 rounded-lg">
                <div class="flex items-start">
                    ...
                    <div>
                        <h4 class="text-sm font-medium text-indigo-900">AI-Powered Categorization</h4>
                        <p class="text-sm text-indigo-700 mt-1">
                            When category or priority is not selected, our AI will automatically analyze the ticket
                            content and suggest the most appropriate values.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-200">
                <a href="<?= $app->url('tickets') ?>" class="px-4 py-2 text-gray-600 hover:text-gray-800">Cancel</a>
                <button type="submit"
