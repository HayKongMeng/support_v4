...
<head>
    ...
    <style>
        ...
        .sync-status-page {
            padding: 15px;
            padding-top: 80px; /* Account for fixed header (64px + buffer) */
            height: 100%;
            overflow-y: auto;
        }
        .toast-notification {
...
<body class="bg-gray-100 min-h-screen" x-data="{ sidebarOpen: true, mobileMenu: false, toasts: window.toastsData }" x-init="window.toastsData = toasts">
    ...
    <div class="flex h-screen overflow-hidden">
        <!-- Sidebar -->
        <aside :class="sidebarOpen ? 'w-64' : 'w-20'" class="hidden lg:flex flex-col bg-slate-800 text-white transition-all duration-300 z-30">
            <!-- Logo -->
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            <!-- Top header -->
            <header class="bg-white shadow-sm h-16 flex items-center justify-between px-4 lg:px-6 relative z-40">
                <div class="flex items-center">
    ...
    // Global function to remove toast
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        if (newCount > this.lastCount && this.lastCount > 0) {
                            // Find new notifications by comparing with previous list
                            const newNotifications = data.data.notifications.filter(n =>
                                !prevNotifications.some(prev => prev.id === n.id)
                            );

                            // Show toast and desktop notification for each new notification
            ...
            showBrowserNotification(n) {
                ...
                try {
                    ...
                    const notification = new Notification(n.title, {
                        ...
                        icon: '/favicon.ico',
                        badge: '/favicon.ico',
                        tag: 'ticket-' + n.id, // Unique tag per ticket
                        ...
                        // Actions for the notification (Chrome supports these)
                        actions: [
                            {
                                action: 'view',
                                title: 'View Ticket'
