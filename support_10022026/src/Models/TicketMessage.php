...
class TicketMessage extends Model
{
    ...
    public function addReply(int $ticketId, int $userId, string $message, bool $isInternal = false, string $source = 'web', array $attachments = []): int
    {
        ...
        // Update ticket's first_response_at if this is from an agent (and not internal note)
        if (!$isInternal) {
            $user = $this->db->selectOne("SELECT role FROM users WHERE id = ?", [$userId]);
            if ($user && in_array($user['role'], ['admin', 'agent', 'front_office_agent', 'back_office_agent'])) {
                $ticket = $this->db->selectOne(
                    "SELECT first_response_at, company_id FROM tickets WHERE id = ?",
                    [$ticketId]
                );
                if ($ticket && !$ticket['first_response_at']) {
                    ...
                    $ticketModel->setCompanyId($ticket['company_id']);
                    $ticketModel->updateFirstResponseSla($ticketId);
                }
            }
        }
