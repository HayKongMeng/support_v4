...
class Ticket extends Model
{
    ...
    public function getFiltered(array $filters = [], int $page = 1, int $perPage = 25, ?array $user = null): array
    {
        ...
        // Role-based filtering: restricted agents can see assigned tickets + unassigned tickets (for pickup)
        if ($user && in_array($user['role'] ?? '', ['front_office_agent', 'back_office_agent'])) {
            $sql .= " AND (t.assigned_to = ? OR t.requester_id = ? OR t.assigned_to IS NULL)";
            $countSql .= " AND (t.assigned_to = ? OR t.requester_id = ? OR t.assigned_to IS NULL)";
            $params[] = $user['id'];
            $params[] = $user['id'];
        }
    ...
    public function getStats(?array $user = null): array
    {
        ...
        // Role-based filtering: restricted agents see assigned + unassigned tickets
        if ($user && in_array($user['role'] ?? '', ['front_office_agent', 'back_office_agent'])) {
            $sql .= " AND (assigned_to = ? OR requester_id = ? OR assigned_to IS NULL)";
            $params[] = $user['id'];
            $params[] = $user['id'];
        }
    ...
    public function create(array $data): int
    {
        ...
        // Calculate and set SLA for the ticket
    ...
    /**
     * Get tickets with SLA about to breach (within next N minutes)
     */
